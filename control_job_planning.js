...
//######################################################################################################################
    ...
    var res_pickup_spots_tmp = concat(res_pickup_spots_cont,res_pickup_spots_links)
      ...
      for ( var x of res_pickup_spots ) { // for each res_pickup_spots with stuff
        for ( var res in x.store ) { // for each resource type in each container's store list
          if ( x.store[res] > 0 || ( x.structureType == STRUCTURE_LINK && x.energy > 0 ) ) { // that is not empty
            ...
            Log.debug("RES " +  " info  " + Hive.upgrader_carry_cap[spawn_num] + " type " + x.structureType,'Planner');
            ...
            Log.debug(x.id + " RES " + res + " " + x.store[res] + " ex jobs: " + job_count.length + " num: " + num_of_jobs_needed + " stType " + x.structureType,'Planner');
