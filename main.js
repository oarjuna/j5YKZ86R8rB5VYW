  ...
  var Hive = {
    ...
    spawn_levels:
                ...
                [// H  D  U  B Cr  Rh C  S  R Sm Sr SH
                  [ 2, 3, 2, 0, 0, 1, 0, 0, 0, 0, 0, 0], // Spawn1
                  [ 2, 3, 2, 1, 0, 2, 0, 0, 0, 0, 0, 0], // Spawn2
                  [ 2, 3, 2, 1, 0, 2, 0, 0, 0, 0, 0, 0], // Spawn3
                  [ 2, 1, 3, 1, 0, 0, 0, 0, 0, 0, 0, 0]  // Spawn4
                ],
    sources:    [
...
*/
  ...
  for ( let job of Hive.memory.job_queue) {
    ...
    if (job.dest_id != undefined ) { var dest_obj = Game.getObjectById(job.dest_id); dest_type = dest_obj.structureType; }
    else { dest_type = 'unassigned'; }

    console.log("JQ: " + job.spawn_name+ " j_id: " + job.id + " type " + job.type + " st: " + job.state + " cr: " + name + " d: " + dest_obj.structureType + " ex: " + job.extra + " tq " + ( Game.time - job.tick_issued));
  }
  console.log("------ END ------");
  //console.log("#-------------------------------------------------------#  H, U, R, B, RH, C, De, EM, S");
  stats.collect_stats();
